...
def _compute_integrity_score(stats: dict[str, int]) -> int:
    ...
    good = stats.get("clean", 0) + stats.get("doi_verified", 0) + stats.get("weak_evidence", 0)
    bad = stats.get("ghost", 0) + stats.get("fabricated", 0) + stats.get("contradicted", 0)
    score = ((good - bad) / total) * 100
